...
housing_randomForest <- randomForest(formula = SalePrice ~ ., data = dat)
```

```{r}
plot(housing_randomForest)

variableImportance <- data.frame(housing_randomForest$importance) %>%
  arrange(desc(IncNodePurity)) %>%
  top_n(25)

variableImportance

ggplot(data = variableImportance, aes(x = reorder(row.names(variableImportance), IncNodePurity), y = IncNodePurity)) + geom_col() + coord_flip() + ggtitle("Top 25 important variables")
```
